...
| 原则 | 说明 | 体现 |
|-----|------|------|
| 简单优先 | MVP 追求最小可行 | Docker Compose 多容器部署、SQLite 存储 |
| 低成本 | 最小化运营开销 | MinIO 本地对象存储、无云存储费用 |
...
│         ▼                       ▼                       ▼                    │
│  ┌─────────────────┐   ┌─────────────────┐   ┌─────────────────────────┐   │
│  │  Go Backend     │   │  MinIO          │   │  Volume Mounts          │   │
│  │  Container      │   │  Container      │   │  (宿主机挂载)            │   │
│  │                 │   │                 │   │                         │   │
│  │  • Gin Router   │   │  • S3 API(9000) │   │  ./data/db → SQLite    │   │
│  │  • JWT认证      │   │  • Console(9001)│   │  ./data/minio → 文件   │   │
│  │  • 频率限制     │   │  • 文件存储     │   │                         │   │
│  │  • 游客排队     │   │                 │   └─────────────────────────┘   │
│  │                 │   └────────┬────────┘                                  │
│  │  ┌───────────┐  │            │                                           │
│  │  │  SQLite   │  │←───────────┘ (Presigned URL 下载)                      │
│  │  └───────────┘  │                                                        │
│  └─────────────────┘                                                        │
│                                                                              │
│  ┌──────────────────────────────────────────────────────────────────────┐   │
...
| 对象存储 | MinIO | latest | S3 兼容、自托管、无云存储费用 |
| 缓存 | go-cache | - | 内存缓存，用于频率限制 |
| JWT | jwt-go | v5 | 用户认证 |
| Web服务器 | Nginx | 1.24+ | 静态资源、反向代理 |
| 容器编排 | Docker Compose | 2.0+ | 多容器管理、一键部署 |
| 短信服务 | 阿里云/腾讯云 | - | 验证码发送 |
...
| 选项 | 优点 | 缺点 | 结论 |
|-----|------|------|------|
| **MinIO** | S3 兼容、自托管、无存储费用 | 需要维护 | ✅ 选用 |
| 阿里云 OSS | 稳定、CDN 集成 | 按量付费、成本高 | ❌ |
| 本地文件系统 | 简单 | 不支持分布式、管理困难 | ❌ |
...
宿主机挂载:
./data/db/         → backend:/app/data/    # SQLite 数据库
./data/minio/      → minio:/data/          # 对象存储文件
```
...
│  │                                                                        │  │
│  │   ./data/db/              → SQLite 数据库                              │  │
│  │   ./data/minio/           → MinIO 对象存储文件                         │  │
│  │                             ├── courses/{id}/intro/   # 课程介绍 MD   │  │
│  │                             └── courses/{id}/resource/ # 课程资源 ZIP  │  │
│  └────────────────────────────────────────────────────────────────────────┘  │
│                                                                              │
│  ┌────────────────────────────────────────────────────────────────────────┐  │
│  │                              外部服务                                   │  │
...
**更新日志**：
- v2.1 (2025-01-17): 新增 MinIO 对象存储架构，更新为 Docker Compose 多容器部署方案
