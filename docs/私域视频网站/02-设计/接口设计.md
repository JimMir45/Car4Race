...
| | `/api/v1/admin/courses` | GET | 是 | 获取课程列表（管理） |
| | `/api/v1/admin/courses/:id/files` | POST | 是 | 上传课程文件 |
| | `/api/v1/admin/courses/:id/files` | GET | 是 | 获取课程文件列表 |
| | `/api/v1/admin/courses/:id/files/:fileId` | DELETE | 是 | 删除课程文件 |
...
#### POST /api/v1/admin/courses/:id/files

上传课程文件（支持 MD 介绍文件和 ZIP 资源文件）。
...
| 字段 | 类型 | 必填 | 说明 |
|-----|------|-----|------|
| file | File | 是 | 文件（.md 或 .zip） |
| file_type | string | 是 | 文件类型：intro（介绍）/ resource（资源） |
...
```json
{
  ...
  "data": {
    ...
    "file_type": "intro",
    "file_name": "课程介绍.md",
    "file_path": "courses/1/intro/课程介绍.md",
    "file_size": 2048,
...
#### GET /api/v1/admin/courses/:id/files
...
#### DELETE /api/v1/admin/courses/:id/files/:fileId
